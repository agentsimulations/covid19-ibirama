...
;FDS: Lucas, please fill with the values you use to run the simulation on your computer ->  World extent for debuging/testing: max-pxcor=?? /max-pycor=??
extensions [ gis ]
globals [
  sublists
  lists
...
civilians-own[
  ...
  age types worker
  n ;; Variable to control at which turn the agent should move ( 0 = Matutino, 1 = Vespertino, 2 = Noturno )
  state infected days
...
to move
  ...
  foreach sort civilians [ the-turtle ->  ;; EI EF1 EF2 EM CEJA UNI ;FDS why 'sort'?
    ask the-turtle [                      ;; Variable to control at which turn the agent should move ( 0 = Matutino, 1 = Vespertino, 2 = Noturno )
      ifelse casa = true [][
      ifelse ticks = ss [                 ;; EF1 e EF2 sao dividos em 0 e 1, CEJA e EM sao dividos em 0;1;2, UNI sempre será 2, EI é 0 mas deve permanecer no turno vespertino
...
;  ][
   let try  workers-isolation-fraction * NUMBER_WORKERS
    ...
    let nbe 4230 ; FDS: magic number? additionally, you must use meaningful variable names ;-)
    set try students-isolation-fraction * nbe
    ask n-of try civilians with [ types != "NE" ][
      set casa true
    ]
